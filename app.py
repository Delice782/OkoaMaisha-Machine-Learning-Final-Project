...
#                 <li><strong>False Negatives:</strong> Only 31 long-stay patients incorrectly flagged as short-stay patients(1.8%)</li>
#             </ul>
#         </div>
#         """, unsafe_allow_html=True)
...
# VERSION WITH LOGIN + SIGNUP PAGES
...
# DATABASE SETUP
def init_database():
    conn = sqlite3.connect('okoamaisha_users.db')
    c = conn.cursor()
    c.execute('''
...
def authenticate_user(username, password):
    try:
        conn = sqlite3.connect('okoamaisha_users.db')
        c = conn.cursor()
        c.execute('SELECT id, username, password_hash, email, full_name FROM users WHERE username = ?', (username,))
        user = c.fetchone()
        conn.close()
        
        if user and verify_password(password, user[2]):
            return True, {'id': user[0], 'username': user[1], 'email': user[3], 'full_name': user[4]}
        return False, None
    except:
        return False, None
...
# LOGIN PAGE
def show_login_page():
    st.markdown("<div style='text-align: center; padding: 2rem 0;'><h1 style='color: #1e3a8a; font-size: 3rem;'>🏥 OkoaMaisha</h1><p style='color: #64748b; font-size: 1.2rem;'>Hospital Length of Stay Predictor</p></div>", unsafe_allow_html=True)
                ...
                if signup_button:
                    if all([full_name, email, new_username, new_password, confirm_password]):
                        if not validate_email(email):
                            ...
                            if success:
                                st.success("🎉 " + message + " Switch to Login tab!")
                            else:
                                st.error("❌ " + message)
                    else:
                        st.error("⚠️ Fill all fields!")
